 ...
 * type-checked. Two targets with the same source identity produce the same
 * type-checking result regardless of module format. The first target
 * type-checked in a source group validates types for all subsequent targets
 * in that group.
 */
export function sourceIdentity(fileNames: readonly string[]): string {
  const filesHash = crypto
    .createHash("sha256")
    .update([...fileNames].sort().join("\0"))
    .digest("hex")
...
/**
 * Compute a "program identity" hash — the resolved program graph identity.
 *
 * Includes both file names and `customConditions` because conditions change
 * how `#platform/*` (subpath) imports are resolved, producing different
 * program graphs even when the initial fileNames list is identical.
 *
 * Two targets with the same program identity emit the same output files,
 * so declarations and transpiled JS can be reused.
 */
export function programIdentity(
  ...
  hash.update([...fileNames].sort().join("\0"));
  if (customConditions && customConditions.length > 0) {
    hash.update("\0conditions:" + [...customConditions].sort().join(","));
  }
...
export async function compileAllTargets(
  ...
  // Key: sourceIdentity hash (fileNames only). Value: outDir of the first target.
  const typeCheckedSources = new Map<string, string>();

  // Track which program graphs have been fully compiled (JS + declarations).
  // Key: programIdentity hash (fileNames + customConditions). Value: outDir.
  ...
  for (const group of groups) {
    ...
    const { fileNames } = group.primary.parsedConfig;
    const { customConditions } = group.primary.parsedConfig.options;

    // Type-check identity: same fileNames → same type-checking result.
    const typeCheckId = sourceIdentity(fileNames);
    const alreadyTypeCheckedOutDir = typeCheckedSources.get(typeCheckId);
    const needsTypeCheck = !alreadyTypeCheckedOutDir;

    // Program identity: same fileNames + customConditions → same resolved
    // import graph → same output files. When conditions differ, imports
    // like #platform/* resolve to different files, so declarations and
    // transpiled JS cannot be reused.
    const progId = programIdentity(fileNames, customConditions);
    const alreadyCompiledOutDir = compiledPrograms.get(progId);
    const canReuseOutput = !!alreadyCompiledOutDir;

    if (needsTypeCheck) {
      typeCheckedSources.set(typeCheckId, group.primary.outDir);
    }
    ...
    if (!needsTypeCheck && canReuseOutput) {
    ...
    if (canReuseOutput && alreadyCompiledOutDir && primaryResult.success) {
